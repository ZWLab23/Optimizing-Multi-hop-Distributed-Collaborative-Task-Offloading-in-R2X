...
from methods.hop_number.datastruct_max_hop import VehicleList, RSUList, TimeSlot, Function
import torch
from methods.hop_number.config_max_hop import VehicularEnvConfig
from methods.hop_number.collaborateDRL import get_collaborate_decision
import math
import numpy as np
import copy
import random
...
class RoadState(gym.Env):
    ...
    def _reset_road(self) -> None:
        ...
        #enumerate函数在循环迭代时返回元素的索引和元素本身
        ##################################################################
            state = np.random.get_state()
            # 设置种子，生成a
            np.random.seed(self.seed + i)

            num_location_changes = np.random.randint(0, 10)
    ...
    def reset(
            ...
            *,
            ...
            # 这是一个可选参数，它允许你指定一个随机数种子。种子用于控制伪随机数生成器的行为，如果你想要在每次重置时获得相同的随机状态
            # ，可以设置种子。通常用于复现实验结果。如果不提供种子，则默认为 None，表示不使用特定的种子。
            return_info: bool = False,
            # 这是一个布尔值参数，它确定是否要在 reset 方法中返回额外的信息。如果设置为 True，则 reset 方
            # 法可能会返回一些关于环境状态的额外信息或元数据，以便在需要时进行分析或记录。如果设置为 False，则只返回环境状态。默认为 False。
            options: Optional[dict] = None,
            # 这是一个可选的字典参数，用于传递其他配置选项。字典可以包含任何其他与 reset 方法相关的配置信息，
            # 具体取决于你的应用程序或环境的需求。如果不需要任何额外的配置选项，可以将其设置为 None。
    ):
    ...
    def get_vehicle_sum_tasks_dict(self):
        sum_tasks_dict = {}
        for i in range(self.vehicle_number):
            sum_tasks = self.vehicle_list.vehicle_list[i].get_sum_tasks()
    ...
    def get_reward_and_function_allocation(self,action,new_task_list):
        ...
        for i in range(self.config.rsu_number):
            task=new_task_list[i].copy()   # [产生该任务的RSU编号，任务大小，任务所需计算资源，任务延迟]
            offloading_number=action_list[i]
            if action_list[i]<=self.rsu_number-1:#如果卸载到RSU上
                offloading_rsu=offloading_rsu+1
                #传输时延
                up_delay=task[1]/self.config.r2r_rate*(abs(i-offloading_number))
                #等待时延
                ...
                # 传输能耗
                up_energy_consumption=task[1]/self.config.r2r_rate*(abs(i-offloading_number))*self.config.rsu_p
                # 计算能耗
            ...
            #卸载到多辆车辆共同协作,这里的数据都是处理完一个任务就更新的
            elif action_list[i]==self.rsu_number:
                offloading_vehicle = offloading_vehicle + 1

                delay_list_node=[]
                energy_consumption_list_node=[]
                current_node=i+self.config.vehicle_number #因为字典里RSU的编号从10开始
                check_last_node =i+self.config.vehicle_number #为了检察训练给出的决策是否有问题
                check_last_node_1 = i + self.config.vehicle_number # 为了检察训练给出的决策是否有问题,有2轮检查
                is_not_execute=0
                ...
                if all_vehicle_number[-1][0] !=self.config.vehicle_number:
                    reward = self.config.RSUDRL_punishment
                    reward_list.append(reward)
                    dealy_list.append(30)
                    energy_consumption_list.append(200)
                    success_task.append(0)
                    # print(i, action_list[i])
                    ...
                    for j in range(len(all_vehicle_number)):#[[8,3],[10, 6]]
                        ...
                        # 当前节点上所需总时延，取最大
                        # print("等待时延：",wait_delay_current_node,"计算时延：",process_delay_current_node,"等待时延+计算时延：",wait_delay_current_node+process_delay_current_node,"传输时延：",up_delay_current_node)
                        all_delay_current_node=max([wait_delay_current_node+process_delay_current_node,up_delay_current_node])
                        delay_list_node.append(all_delay_current_node)
                        #当前节点传输数据到下一跳的传输能耗
                        if all_vehicle_number[j][0] == self.vehicle_number:
                            up_energy_consumption_node=0
                        else:
                            if current_node==i+self.config.vehicle_number:
                        ...
                        energy_consumption_list_node.append(all_energy_consumption_current_node)
                        #添加任务放置的任务
                    ...
                    # print("结束")
                    all_delay=sum(delay_list_node)
                    all_energy_consumption=sum(energy_consumption_list_node)
                    ...
                    if all_delay <= task[3]:
                        ...
                        success_task.append(1)
                        # 只有成功卸载,才会进行任务放置
                        self.rsu_list.rsu_list[i].get_task_list().add_task_list(allocaion_tasksize[0])
...
#####################################################未测试##############################################################
    ...
    def update_road(self):
        ...
        # 更新所有车辆的历史速度集合
        for i in range(self.vehicle_number):
            self.vehicle_list.vehicle_list[i].change_history_speed_list()
        # 更新所有车辆的位置
            self.vehicle_list.vehicle_list[i].change_location()
        # 更新所有车辆的速度
            self.vehicle_list.vehicle_list[i].change_speed()
        #更新是否发送信标的beacon
    ...
    def step(self,action):
        ...
        # 奖励值更新
        reward,all_task_dealy,all_task_energy_consumption ,complete_number,offloading_rsu,offloading_vehicle,offloading_cloud \
            = self.get_reward_and_function_allocation(action,new_task_list)
        # done更新

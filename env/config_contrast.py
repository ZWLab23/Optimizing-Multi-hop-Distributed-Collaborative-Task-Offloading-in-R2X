...
class VehicularEnvConfig:
    def __init__(self):
        # 道路信息
        self.road_range: int = 900  # 道路长度
        self.road_width: int = 50  # 道路宽度

        # 时间信息
        self.time_slot_start: int = 0
        self.time_slot_end: int = 59
        # 任务信息相关（要处理的任务）
        ...
        self.Function_min_task_delay: int = 5# 任务的最小延迟20s
        self.Function_max_task_delay: int = 15  # 任务的最大延迟25s

        # 任务队列相关（每个卸载对象自己产生的任务，即自身到达任务）
        self.min_rsuself_task_number: int = 4    #RSU最小任务个数
        ...
        self.min_vehicleself_task_number: int = 2    #车辆最小任务个数,用于生成初始任务的个数
        self.max_vehicleself_task_number: int = 3   #车辆最大任务个数,用于生成初始任务的个数
        self.min_vehicleself_task_datasize: float = 2  # 2 MB 每个任务的最小数据大小
        self.max_vehicleself_task_datasize: float = 4  # 4 MB   每个任务的最大数据大小

        # 车辆相关
        # self.min_vehicle_speed: int = 20 #车辆行驶的最小速度
        # self.max_vehicle_speed: int = 20 #车辆行驶的最大速度
        self.vehicle_speed: int = 20 #车辆行驶的最大速度
        ...
        self.collaborateDRL_reward_weight = 0.5
        self.RSUDRL_reward_weight = 0.5# 计算奖励时时延和能耗的系数
        ...
        # contrast_Lyapunov相关
        # 模型信息
        self.w_Lyapunov=100  #50,100,200,400
        self.reward_threshold=-2500 #dict = {"weight_1": -500, "weight_2": -500, "weight_3": -500, "weight_4": -500}

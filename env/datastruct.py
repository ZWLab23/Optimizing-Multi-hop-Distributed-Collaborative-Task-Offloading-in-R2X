...
class Vehicle(object):
    ...
    def __init__(
        ...
        np.random.set_state(state)
        # x坐标
        self._vehicle_x_location = self._vehicle_x_initial_location.copy()
        ...
        # 设置种子，生成a
        np.random.seed(self._seed)
        self._vehicle_initial_speed = np.random.randint(self._min_vehicle_speed, self._max_vehicle_speed,1)[0]  # 车辆速度
        np.random.set_state(state)

        if  self._vehicle_x_initial_location==0:
            self._vehicle_initial_speed = self._vehicle_initial_speed
        else:
            self._vehicle_initial_speed = -self._vehicle_initial_speed
        #生成速度
        self._vehicle_speed=self._vehicle_initial_speed.copy()
        ...
        # 设置种子，生成a
        np.random.seed(self._seed)
        self._history_speed = np.random.randint(self._min_vehicle_speed, self._max_vehicle_speed,self._history_data_number)
        np.random.set_state(state)
        self._vehicle_initial_history_speed_list = self._history_speed.tolist()
        if  self._vehicle_x_initial_location==self._road_range:
            self._vehicle_initial_history_speed_list =[-x for x in self._vehicle_initial_history_speed_list]
...
#######################################################还在改
    ...
    def change_beacon_flag(self) -> list:
        if self._initial_beacon_flag==1:
            if((self._life_time)%(VehicularEnvConfig().flag_cycle+1)==0):
                self._beacon_flag=1
            else:
                self._beacon_flag = -1
        if self._initial_beacon_flag==-1:
            if((self._life_time+1)%(VehicularEnvConfig().flag_cycle+1)==0):
                self._beacon_flag=1
            else:
                self._beacon_flag = -1
        # return self._beacon_flag

    def change_life_time(self) -> list:
